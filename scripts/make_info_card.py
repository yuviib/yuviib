...
# ("sec", title)               -> blue "ΓÇö title ΓÇö" rule
# ("bul", text)                -> green dot + light text
# ("gap",)                     -> vertical space
ROWS = [
    ("host",),
    ("kv", "Now", "SWE Intern @ Nokia | Incoming @ RBC"),
    ("kv", "Also", "SWE @ Carleton"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Languages", "Python, Java, C++, Ruby, JavaScript"),
    ("kv", "AI / ML", "PyTorch, LangChain, LLMs"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "RavenMap"),
    ("bul", "Resonance"),
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">yuviib@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">yuviib</tspan><tspan fill="{MUTED}">@</tspan>'
